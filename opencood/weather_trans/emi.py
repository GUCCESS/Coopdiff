...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='LiDAR emi')
    ...
    parser.add_argument('-d', '--dst_folder', help='savefolder of dataset', type=str,
                        default='./save_root/emi/light')  # ['light','moderate','heavy']
...
def drop_beams(pointcloud, num_beams_to_drop):
    ...
    elif num_beams_to_drop == 8:
        # 新增：每8条光束丢弃1条
        to_drop = np.arange(1, 64, 8)
    else:
...
def apply_emi_to_numpy(point_cloud_np):  
    
    np.random.seed(420)

    num_beams_to_drop = 32 #8 16 32 48
