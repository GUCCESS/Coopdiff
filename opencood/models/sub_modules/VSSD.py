...
def ssd_minimal_discrete(X, A, B, C, block_len, initial_states=None):
    """
    Arguments:
        X: (batch, length, n_heads, d_head)   V
        A: (batch, length, n_heads)
        B: (batch, length, n_heads, d_state)   K
        C: (batch, length, n_heads, d_state)   Q
    Return:
    ...
    A_cumsum = torch.cumsum(A, dim=-1)  # 累加和

    # 1. Compute the output for each intra-chunk (diagonal blocks)
    # 计算每个块内（对角块）的输出
    L = torch.exp(segsum(A))  # segsum处理A，上三角为inf。 倒三角矩阵 (b, h, c, l) -> (b, h, c, l, l)

    # C 和 B 的维度 n 被消掉，因为这两者通过 n 进行矩阵乘法 tp:(b, c, l, h, s)
    # tp 和 L 进行 消除s 得到(b, c, l, h)
    # 与X 消除s
    # print('X ', X.shape)
    # print('L ', L.shape)
    # print('B ', B.shape)
    ...
    #print('Y_diag', Y_diag.shape)
    # 2. Compute the state for each intra-chunk
    # 计算每个块内的状态 （低秩分解的非对角块的右项；B项）
    decay_states = torch.exp((A_cumsum[:, :, :, -1:] - A_cumsum))
    states = torch.einsum("bclhn,bhcl,bclhp->bchpn", B, decay_states, X)

    # 3. 计算块间SSM递归；在块边界生成正确的SSM状态
    # （非对角块分解的中间项；A项）
...
class Mamba2(nn.Module):
    def __init__(
            self,
            d_model,
            d_conv=3,  # 卷积核default to 3 for 2D
            conv_init=None,
            expand=2,
            headdim=64,  # default to 64 每个注意力头的维度
            ngroups=1,  # 注意力头组数
            # dt和A的初始化限制
            A_init_range=(1, 16),
            dt_min=0.001,
            ...
            learnable_init_states=False,  # 否使用可学习的初始化状态
            activation="silu",  # default to silu
            bias=False,
            conv_bias=True,
            # Fused kernel and sharding options
            chunk_size=64,  # 用于内存高效路径的分块大小
            use_mem_eff_path=False,  # default to False, for custom implementation
            layer_idx=None,  # Absorb kwarg for general module 记录层索引的参数
            device=None,
            dtype=None,
        ...
        self.nheads = self.d_inner // self.headdim  # 头的个数

        self.dt_limit = dt_limit
        self.learnable_init_states = learnable_init_states
        self.activation = activation

        # convert chunk_size to triton.language.int32 计算时处理的块大小
        ...
        # linear attention duality 是否使用线性注意力双重性
    ...
    def non_casual_linear_attn(self, x, dt, A, B, C, D, H=None, W=None):
        ...
        if self.ngroups == 1:
            ...
            dstate = dstate // self.ngroups  # dstate相当于C
    ...
    def forward(self, u, H=None, W=None, seq=True):
        """
        u: (B,C,H,W)
        ...
        #print('u ',u.shape)
        zxbcdt = self.in_proj(u)  # (B, L, d_in_proj)  2 * self.d_inner + 2 * self.ngroups * self.d_state + self.nheads
        ...
        if self.linear_attn_duality:
            ...
            if True:
                ...
                x = b_l_hp2b_l_h_p(x, p=self.headdim).chunk(2, dim=-2)
                B = b_l_gn2b_l_g_n(B, g=self.ngroups).chunk(2, dim=-2)
                C = b_l_gn2b_l_g_n(C, g=self.ngroups).chunk(2, dim=-2)
                ...
                #print(y_forward.shape, y_backward.shape)
...
class VMAMBA2Block(nn.Module):
    ...
    def __init__(self, dim, input_resolution=[100, 252], num_heads=16, mlp_ratio=4., qkv_bias=True, drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, ssd_expansion=2, ssd_ngroups=2, ssd_chunk_size=256,
    ...
    def forward(self, x, H=None, W=None, hx=None):
        #print('x in vmamba2', x.shape)
        B, L, C = x.shape
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, mlp_ratio=4., qkv_bias=True, drop=0.,
                 ...
                 ssd_expansion=2, ssd_ngroups=2, ssd_chunk_size=256, linear_attn_duality=False, d_state=64, **kwargs):
...
class VMAMBA2(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 ssd_expansion=2, ssd_ngroups=2, ssd_chunk_size=256,
                 linear_attn_duality=True, d_state=16, **kwargs):
        super().__init__()
        ...
        if self.simple_patch_embed:
            self.patch_embed = SimpleStem(img_size=img_size, patch_size=patch_size, in_chans=in_chans,
                                          embed_dim=embed_dim)
        else:
...
class Backbone_VMAMBA2(VMAMBA2):
    ...
    def forward(self, x):
        ...
        H, W = x.shape[-2:]
        print('Backbone_VMAMBA2 x:', x.shape)
        x = self.patch_embed(x)
        print('Backbone_VMAMBA2 x:', x.shape)
        if self.simple_patch_embed:
            H, W = H // 4, W // 4
        ...
        for i, layer in enumerate(self.layers):
            ...
            print('Backbone_VMAMBA2 i o x:', i, o.shape, x.shape)
...
class CMamba2(nn.Module):
    def __init__(
        ...
        self.in_proj_zxbdt = nn.Linear(self.d_model*frame, int(d_in_proj), bias=bias, **factory_kwargs)  #
        self.in_proj_c = nn.Linear(self.d_model, self.ngroups * self.d_state)
    ...
    def forward(self, u, his_u, H=None, W=None, seq=True):
        """
        u: B L C
        his_u: B L C*num
        Returns: same shape as u
        """
        batch, seqlen, dim = u.shape
        #c: self.ngroups * self.d_state 由 x出。其余由his_u出
        c = self.in_proj_c(u) # self.ngroups * self.d_state
        zxbdt = self.in_proj_zxbdt(his_u)  # (B, L, d_in_proj)  2 * self.d_inner +  self.ngroups * self.d_state + self.nheads
        ...
        xBC = torch.cat((xB,c),dim=2) # self.d_inner +  2*self.ngroups * self.d_state
        ...
        token_importance = torch.norm(x, dim=-1)  # (B, L)
        # Step 2: Use topk to retain top 80% of the most important tokens
        k = int(0.8 * seqlen)  # Retain top 80% of tokens
        topk_values, topk_indices = torch.topk(token_importance, k=k, dim=1)

        # Create mask: 1 for top 80%, 0 for the remaining 20%
        mask = torch.zeros_like(token_importance, dtype=torch.float)
        mask.scatter_(1, topk_indices, 1.0)  # Mark top 80% as 1
        mask = mask.unsqueeze(-1).expand_as(x)  # Expand the mask to match x's shape

        # Step 3: Apply the mask to filter out less important tokens
        x = x * mask  # (B, L, d_inner)
        B = B * mask
        C = C * mask
...
class CVMAMBA2Block(nn.Module):
    ...
    def __init__(self, dim, input_resolution=[48,176], num_heads=16, mlp_ratio=4., qkv_bias=True, drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, ssd_expansion=2, ssd_ngroups=2, ssd_chunk_size=256,
                 linear_attn_duality=False, Bi = True, d_state=16,frame=1, **kwargs):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        self.num_heads = num_heads
        ...
        if kwargs.get('attn_type', 'mamba2') == 'standard':
            ...
            self.attn = CMamba2(d_model=dim, expand=ssd_expansion, headdim=dim * ssd_expansion // num_heads,
                               ngroups=ssd_ngroups, chunk_size=ssd_chunk_size, Bi = Bi,
                               linear_attn_duality=linear_attn_duality, d_state=d_state,frame=frame, **kwargs)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()

        self.cpe2 = nn.Conv2d(dim, dim, 3, padding=1, groups=dim)
        self.cpe3 = nn.Conv2d(dim*frame, dim*frame, 3, padding=1, groups=dim*frame)
        self.norm2 = norm_layer(dim)

        self.mlp = Mlp(in_features=dim, hidden_features=int(dim * mlp_ratio), act_layer=act_layer, drop=drop)

    def forward(self, x, his_x, H=None, W=None):
        B, L, C = x.shape

        # print('x',x.shape)
        # print('his',his_x.shape)
        ...
        x = x + self.cpe1(x.reshape(B, H, W, C).permute(0, 3, 1, 2)).flatten(2).permute(0, 2, 1)
        his_x = his_x + self.cpe3(his_x.reshape(B, H, W, -1).permute(0, 3, 1, 2)).flatten(2).permute(0, 2, 1)
        ...
        # print('x ',x.shape)
        # print('his x ',his_x.shape)

        # SSD or Standard Attention
        x = self.attn(x, his_x, H, W)
        x = shortcut + self.drop_path(x)
        x = x + self.cpe2(x.reshape(B, H, W, C).permute(0, 3, 1, 2)).flatten(2).permute(0, 2, 1)

        # FFN
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        #print('out in CVMAMBA2Block', x.shape)
        return x
...
#     bakcbone = Backbone_VMAMBA2(linear_attn_duality=False, ssd_chunk_size=32).cuda()
#     # backbone2 = VMAMBA2Block(linear_attn_duality=False, ssd_chunk_size=32)
#
#     x = torch.randn(1, 3, 32 * 64, 32 * 16).cuda()
#     # Flops, params = profile(bakcbone, inputs=(x,))  # macs
#     # print('Flops: % .4fG' % (Flops / 1000000000))  # 计算量
#     # print('params参数量: % .4fM' % (params / 1000000))
#     # ys = bakcbone(x)
#     # for i, y in enumerate(ys):
#     #     print(i, y.shape)
#     flops, params = get_model_complexity_info(bakcbone, (3, 32 * 64, 32 * 16), as_strings=True,
#                                               print_per_layer_stat=True)
#     print("%s |%s |%s" % ('modelll', flops, params))
