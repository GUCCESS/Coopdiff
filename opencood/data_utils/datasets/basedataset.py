...
from opencood.weather_trans.emi import apply_emi_to_numpy
from opencood.weather_trans.wet_ground import ground_water_augmentation
from opencood.weather_trans.echo import apply_incomplete_echo_by_height #apply_incomplete_echo_by_cluster
...
class BaseDataset(Dataset):
    ...
    def retrieve_base_data(self, idx, cur_ego_pose_flag=True):
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            # 干净点云*
            lidar_np_clean = pcd_utils.pcd_to_np(cav_content[timestamp_key_delay]['lidar'])
            data[cav_id]['lidar_np_clean'] = lidar_np_clean
            data[cav_id]['transformation_matrix_clean'] = data[cav_id]['params']['gt_transformation_matrix'] #'transformation_matrix'
            ...
            # data[cav_id]['lidar_np'] = apply_emi_to_numpy(data[cav_id]['lidar_np'])
            # data[cav_id]['lidar_np'] = ground_water_augmentation(data[cav_id]['lidar_np'])
            # data[cav_id]['lidar_np'] = apply_incomplete_echo_by_height(data[cav_id]['lidar_np'])
            
            # if self.train:
            #     augmentation_functions = [
            #             apply_beam_missing_to_numpy,
            #             apply_motion_blur_to_numpy,
            #             apply_fog_to_numpy,
            #             apply_cross_talk_to_numpy,
            #             apply_emi_to_numpy,
            #             ground_water_augmentation,
            #             apply_incomplete_echo_by_height
            #         ]
            #     # 随机选择一个增强函数
            #     import random
            #     chosen_augmentation = random.choice(augmentation_functions)
            #     # 应用选择的增强
            #     data[cav_id]['lidar_np'] = chosen_augmentation(data[cav_id]['lidar_np'])
            # else:
            #     # data[cav_id]['lidar_np'] = apply_fog_to_numpy(data[cav_id]['lidar_np'])
            #     pass
    ...
    def add_loc_noise(self, pose, xyz_std, ryp_std):
        ...
        # np.random.seed(None)
        # xyz_std = np.random.choice([0, 0.2, 0.4, 0.6,0.8])
        # ryp_std = np.random.choice([0, 0.2, 0.4, 0.6, 0.8])
        # # print('ryp_std:', ryp_std)
